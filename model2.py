    ...
    def __init__(
        ...
        self.prms = nn.ModuleList([
            PRM(in_chans=embed_dims[idx - 1] if not idx == 0 else 64,
                ...
                kernel_size=3,
                downsample_ratio=2 if not idx == 0 else 1)
            for idx in range(self.num_stages)
        ])
...
class CrossAtt(nn.Module):
    ...
    def forward(self, x1, x2):
        x11 = self.crlayer1(x1)
        x22 = self.crlayer2(x2)
        ...
        x1 = x1.permute(0, 2, 3, 1).reshape(B, N, C)
        qkv1 = (self.qkv1(x1).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4))
        q1, k1, v1 = qkv1[0], qkv1[1], qkv1[2]

        x2 = x2.permute(0, 2, 3, 1).reshape(B, N, C)
        qkv2 = (self.qkv2(x2).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4))
        q2, k2, v2 = qkv2[0], qkv2[1], qkv2[2]

        k1_T_dot_q1 = torch.matmul(q1, k1.transpose(-2, -1)) / self.scale
        p_attn1 = torch.softmax(k1_T_dot_q1, dim=-1)
        att1 = torch.matmul(p_attn1, v2)

        k2_T_dot_q2 = torch.matmul(q2, k2.transpose(-2, -1)) / self.scale
        p_attn2 = torch.softmax(k2_T_dot_q2, dim=-1)
        att2 = torch.matmul(p_attn2, v1)

        att1 = att1.transpose(1, 2).reshape(B, C, H, W)
        att2 = att2.transpose(1, 2).reshape(B, C, H, W)
...
class Get_diff(nn.Module):
    def __init__(self, emb_dim=288):
        super().__init__()
        self.multi_conv1 = nn.Sequential(
            nn.Conv2d(emb_dim, emb_dim // 4, 1, 1, 0),
            nn.Conv2d(emb_dim // 4, emb_dim // 4, 3, 1, 1),
            nn.Dropout(p=0.5),
        ...
        self.multi_conv2 = nn.Sequential(
            nn.Conv2d(emb_dim, emb_dim // 4, 1, 1, 0),
            nn.Conv2d(emb_dim // 4, emb_dim // 4, 5, 1, 2),
            nn.Dropout(p=0.5),
            nn.BatchNorm2d(emb_dim // 4),
            nn.ReLU(),
        )
        self.conv = nn.Conv2d(emb_dim // 2, emb_dim, 1, 1, 0)
        self.sigmiod = nn.Sigmoid()

    def forward(self, x1, x2):
        diff_f = x2 - x1
        diff1 = self.multi_conv1(diff_f)
        diff2 = self.multi_conv2(diff_f)
        diff = torch.cat([diff1, diff2], dim=1)
        diff = self.conv(diff)
        diff = self.sigmiod(diff)
        diff = diff * (x1 + x2)

        return diff

class Fusion(nn.Module):
    def __init__(self, emd_dim=288):
        super().__init__()
        self.global_attn = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(emd_dim, emd_dim // 4, 1, 1, 0),
            nn.BatchNorm2d(emd_dim // 4),
            nn.ReLU(),
            nn.Conv2d(emd_dim // 4,  emd_dim, 1, 1, 0),
            nn.Sigmoid()
        )
        self.local_attn = nn.Sequential(
            nn.Conv2d(emd_dim, emd_dim, 3, 1, 1),
            nn.Conv2d(emd_dim, emd_dim // 4, 1, 1, 0),
            nn.BatchNorm2d(emd_dim//4),
            nn.ReLU(),
            nn.Conv2d(emd_dim // 4, emd_dim, 1, 1, 0),
    ...
    def forward(self, x):
        x = x + x*self.global_attn(x)
        x = x + x*self.local_attn(x)

        return x


class MutiBranch(nn.Module):
    def __init__(self, embed_dim=288, num_class=3, img_size=512):
        super(MutiBranch, self).__init__()
        self.branch1 = MPViT(in_chans=1,
                             img_size=img_size,
                             num_stages=4,
                             num_path=[2, 3, 3, 3],
                             num_layers=[1, 3, 6, 3],
        ...
        self.branch3 = MPViT(in_chans=1,
                             ...
                             mlp_ratios=[4, 4, 4, 4],
                             num_heads=[8, 8, 8, 8], )
        self.fusion = Fusion(emd_dim=embed_dim)
        self.diff1 = Get_diff()
        self.diff2 = Get_diff()
        self.diff3 = Get_diff()
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.cls = nn.Linear(embed_dim, num_class)
        self.corss_att = CrossAtt(dim=embed_dim)
        self.conv_o = nn.Conv2d(embed_dim*3, embed_dim, 1, 1, 0)
        self.conv_d = nn.Conv2d(embed_dim*3, embed_dim, 1, 1, 0)

    def forward(self, x1, x2, x3):
        x1, output1 = self.branch1(x1)
        x2, output2 = self.branch2(x2)
        x3, output3 = self.branch3(x3)
...
if __name__ == "__main__":
    ...
    model = MutiBranch()
    inputs = torch.randn(1, 3, 1, 512, 512)
    flops, params = profile(model, inputs=(inputs[:, 0, :], inputs[:, 1, :], inputs[:, 2, :]))
    print(flops, params)
